...
import { UserJwtPayload, AdminJwtPayload } from "../utils/interfaces/utils.interfaces";
...
/**
 * @Responsibility:  Middleware authentication for admin
 *
 * @param req
 * @param res
 * @param next
...
const isAdmin = async (req: Request, res: AdditionalResponse, next: NextFunction) => {
  let { authorization } = req.headers;
  const { userId } = req.body;
  ...
  // decode jwt token from req header
  const decode = jwt.verify(authorization as string, process.env.JWT_SECRET_KEY as string, (err, decoded) => decoded) as unknown as AdminJwtPayload;
  ...
  try {
    const thisIsAdmin = decode.isStaff ? true : false;
    res.admin = thisIsAdmin;
    return next();
  } catch (error) {
    // console.log(error);
    return ResponseHandler.sendFatalError({ res });
...
export default { signupValidation, loginValidation, authenticateUser, isAdmin };
